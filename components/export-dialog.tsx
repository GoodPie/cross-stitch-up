...
export function ExportDialog({ open, onOpenChange, canvas, filename }: ExportDialogProps) {
  ...
  // Initialize pixel dimensions from canvas
  useEffect(() => {
    if (canvas) {
  ...
  const handleExport = () => {
    ...
    if (format === "png") {
      ...
      downloadAsPdf(canvas, filename, {
        sizeMode: sizeMode === "original" ? "fit" : sizeMode,
        ...options,
      })
    }
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-3">
            ...
            <RadioGroup value={sizeMode} onValueChange={(v) => setSizeMode(v as SizeMode)} className="space-y-2">
              <div className="flex items-center space-x-2">
              ...
              <div className="flex items-center space-x-2">
                ...
                <Label htmlFor="size-pixels" className="cursor-pointer">Custom pixel dimensions</Label>
              </div>
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="print" id="size-print" />
                <Label htmlFor="size-print" className="cursor-pointer">Print size (inches)</Label>
              </div>
            </RadioGroup>
          </div>
            ...
            <div className="space-y-3 pl-6 border-l-2 border-border">
              <div className="flex gap-3">
                <div className="flex-1 space-y-1">
                  <Label htmlFor="pixel-width" className="text-xs">Width (px)</Label>
                  <Input
                    id="pixel-width"
                    type="number"
                    value={pixelWidth}
                ...
                <div className="flex-1 space-y-1">
                  <Label htmlFor="pixel-height" className="text-xs">Height (px)</Label>
                  <Input
                    id="pixel-height"
                    type="number"
                    value={pixelHeight}
              ...
              <div className="flex items-center space-x-2">
                ...
                />
                <Label htmlFor="aspect-ratio-pixels" className="text-xs cursor-pointer">
          ...
          {sizeMode === "print" && (
            <div className="space-y-3 pl-6 border-l-2 border-border">
              <div className="flex gap-3">
                <div className="flex-1 space-y-1">
                  <Label htmlFor="print-width" className="text-xs">Width (inches)</Label>
                  <Input
                    id="print-width"
                    type="number"
                    value={printWidth}
                ...
                <div className="flex-1 space-y-1">
                  <Label htmlFor="print-height" className="text-xs">Height (inches)</Label>
                  <Input
                    id="print-height"
                    type="number"
                    value={printHeight}
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => setMaintainAspectRatio(checked === true)}
                />
                <Label htmlFor="aspect-ratio-print" className="text-xs cursor-pointer">
                  Maintain aspect ratio
                </Label>
              </div>
            </div>
          ...
          {outputDimensions && (
            <div className="bg-accent/50 rounded-lg p-3 text-sm">
              ...
              <span className="font-medium">
                {outputDimensions.width.toLocaleString()} x {outputDimensions.height.toLocaleString()} pixels
              </span>
              {sizeMode === "print" && (
        ...
        <div className="flex justify-end gap-3">
          ...
          <Button onClick={handleExport} className="gap-2">
            {format === "png" ? <FileImage className="w-4 h-4" /> : <FileText className="w-4 h-4" />}
